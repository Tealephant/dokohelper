    ...
    def GetWinner(self, mode):
        if self.IsCompletet():
            ...
            for i in range(1, 4):
                ...
                if not highestCard.GetTrump():
                    #highest card is not trump
                    if not currentCard.GetTrump():
                        #card is not trump
...
class Round:
    ...
    def GetPlayerPoints(self, nr):
        return self.players[nr].GetPoints()
    ...
    def GetTeamPoints(self):
        ...
        for player in self.players:
            ...
            if playerteam == 1:
                points[0] += player.GetPoints()
            elif playerteam == 2:
                points[1] += player.GetPoints()
        return points
    ...
    def UpdatePlayerPoints(self):
        ...
        for trick in self.tricks:
            if trick.IsCompletet():
                ...
                playerPoints[winner.GetNr()] += trick.GetPoints()
        #update points
        for player in self.players:
            player.SetPoints(playerPoints[player.GetNr()])
        ...
        if mode == "normal" or mode == "normals":
            ...
            for trick in self.tricks:
                for i in range(len(trick.cards)):
                    if trick.cards[i].GetType() == "CQ":
                        #player who played this card is Re
                        pos = trick.GetCardPlayerIndex(i)
                        self.players[pos].SetTeam(1)
                        self.players[pos].SetLocked(True)
                        cqPlayed += 1
...
class Player:
    def __init__(self, nr, name):
        self.__nr = nr
        self.__name = name
        self.__roundPoints = 0
        self.__team = 0 #0: undecided, 1: Re, 2: Contra
        self.__locked = False #is the team locked?
...
def cmpCardByRank(card1, card2, reference):
    return reference.index(card1.GetType()) < reference.index(card2.GetType())
